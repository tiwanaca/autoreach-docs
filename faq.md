...
Yes. Connect a Gmail or Outlook account in **Accounts > Email**, then add an email step to any sequence. Subject and body support the same template variables and AI personalization that DMs use. Replies appear in the unified Inbox alongside X and LinkedIn conversations. See [Email Channel](outreach/email-channel.md).
...
No. Email has its own per-sequence daily counter (default 50) that's separate from the unified daily action limit covering DMs, likes, comments, follows, and connection requests. Each mailbox is also rate-paced at one send at a time and up to 10 per minute to keep delivery clean.
