...
- **Calendly**: Requires a paid Calendly subscription. Paste your booking link, paste a Calendly Personal Access Token, click **Auto-fetch** to retrieve your Organization URI, then click **Register webhook** — AutoReach creates the webhook for you. **No form field setup needed** — AutoReach now uses invisible URL-based attribution (`utm_content`), so bookers see only Calendly's default name + email fields.
...
See [Meetings](../meetings/meetings.md) for full details.

> **Note:** Webhooks are optional but strongly recommended. Without one, AutoReach can still inject your booking link into messages, but it can't automatically detect when a meeting is booked — you'll be tracking bookings in a spreadsheet. With a webhook, AutoReach auto-matches bookings to leads via the URL tracking parameter (or falls back to email matching).
