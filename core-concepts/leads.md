...
- **Intent & fit signals** extracted from content
...
Upload customer CSV and let AutoReach find similar profiles on LinkedIn.
...
Bulk import leads from spreadsheet with X handle, LinkedIn URL, or email.

### 7. Lead Pool & RAG Matching
...
{% hint style="info" %}
**Lead Sources & Speed:** Intent-based sources (Tweet Search, LinkedIn Content Search) move leads through enrichment fastest. CSV imports take longest because they skip the initial X/LinkedIn discovery step.
{% endhint %}
...
### not_scored
A newly added lead that hasn't completed enrichment or initial scoring yet. No Buyer Intelligence data available.
...
### disqualified
**Both fit_score < 15 AND intent_score < 15.** AutoReach automatically removes these leads from your database (they're permanently disqualified as a bad fit).
...
**Example:** You search for "moving from Salesforce to HubSpot" on LinkedIn and find Alice. AutoReach automatically finds Alice's X profile (@alice_tweets), looks up her recent tweets, checks for mentions of Salesforce/HubSpot, and uses that social activity as additional intent signal.
...
7. **Removed** → You manually removed from sequence

## Best Practices

{% hint style="info" %}
**Quality Over Quantity:** 100 highly-scored leads from LinkedIn job search will outperform 1000 random CSV imports. Focus on signal-rich sources first.
{% endhint %}
