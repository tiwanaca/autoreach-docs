...
You set the primary mailbox in **Accounts > Email** by clicking **Make Primary** on the account you want as default.
...
Email has its **own** per-sequence daily limit, separate from the unified daily action limit that covers likes, follows, DMs, comments, and connection requests.

| Setting | Default |
|---|---|
| Daily email sends per sequence | 50 |
| Unified daily actions per sequence (DMs, likes, etc.) | 20 |

Each mailbox is also rate-paced at the provider level to keep delivery clean:
